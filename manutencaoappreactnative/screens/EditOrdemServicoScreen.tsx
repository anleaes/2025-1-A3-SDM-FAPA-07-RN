...
type Props = DrawerScreenProps<DrawerParamList, 'EditOrdemServico'>;

const EditOrdemServicoScreen = ({ route, navigation }: Props) => {
  if (!route.params || !route.params.ordem) {
    navigation.navigate('OrdemServicos');
    return null;
  }
  const { ordem } = route.params;
  const [dataFechamento, setDataFechamento] = useState(ordem.data_fechamento);
  const [descricao, setDescricao] = useState(ordem.descricao);
  const [status, setStatus] = useState(ordem.status || 'aberta');
  const [cliente, setCliente] = useState(ordem.cliente?.id ? String(ordem.cliente.id) : '');
  const [endereco, setEndereco] = useState(ordem.endereco?.id ? String(ordem.endereco.id) : '');
  const [tecnico, setTecnico] = useState(ordem.tecnico?.id ? String(ordem.tecnico.id) : '');
  const [servico, setServico] = useState(ordem.servico?.id ? String(ordem.servico.id) : '');
  const [tipoDeServico, setTipoDeServico] = useState(ordem.tipo_de_servico?.id ? String(ordem.tipo_de_servico.id) : '');
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://localhost:8000/ordemServico/${ordem.id}/`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        throw new Error(errText || 'Erro ao salvar ordem de serviço');
      }
      navigation.navigate('OrdemServicos');
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Editar Ordem de Serviço</Text>
...
export default EditOrdemServicoScreen;
