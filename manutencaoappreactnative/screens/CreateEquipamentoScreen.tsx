...
type Props = DrawerScreenProps<DrawerParamList, 'CreateEquipamento'>;

const CreateEquipamentoScreen = ({ navigation }: Props) => {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('http://localhost:8000/equipamento/', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(errText || 'Erro ao criar equipamento');
      }
      setModelo('');
      setMarca('');
      setNumeroSerie('');
...
export default CreateEquipamentoScreen;
