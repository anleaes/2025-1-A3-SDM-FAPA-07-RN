...
type Props = DrawerScreenProps<DrawerParamList, 'EditOrdemServicoItem'>;

const EditOrdemServicoItemScreen = ({ route, navigation }: Props) => {
  ...
  const [ordemServico, setOrdemServico] = useState(item.ordem_servico?.id ? String(item.ordem_servico.id) : '');
  const [servico, setServico] = useState(item.servico?.id ? String(item.servico.id) : '');
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://localhost:8000/ordemServicoItem/${item.id}/`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        throw new Error(errText || 'Erro ao salvar item');
      }
      navigation.navigate('OrdemServicoItens');
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Editar Item da Ordem de Serviço</Text>
...
export default EditOrdemServicoItemScreen;
